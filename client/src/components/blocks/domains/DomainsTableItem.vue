<template>
  ...
  <TableCell>{{ new Date(parseInt(domain.created_at) * 1000).toDateString() }}</TableCell>
  <TableCell>{{ new Date(parseInt(domain.renewal_at) * 1000).toDateString() }}</TableCell>
  <TableCell class="flex justify-end">
    <Dropdown @on-click="isDropdownOpen = !isDropdownOpen" :is-open="isDropdownOpen">
      <template #items>
        <DropdownItem :href="`domains/${domain.name}/a`" text="A Records"/>
        <DropdownItem :href="`domains/${domain.name}/aaaa`" text="AAAA Records"/>
        <DropdownItem :href="`domains/${domain.name}/cname`" text="CNAME Records"/>
